...
export class Game {
  ...
  startLevel(index) {
    ...
    this.board = new Board(8, this.level.tileWeights);
    this.score = 0;
  ...
  async trySwap(r1, c1, r2, c2) {
    ...
    // 甩锅大法：锅移到边缘
    const potOnEdge =
      (tile1 === TILE.POT && this.board.isOnEdge(r2, c2)) ||
      (tile2 === TILE.POT && this.board.isOnEdge(r1, c1));

    if (potOnEdge) {
      const potR = tile1 === TILE.POT ? r2 : r1;
      const potC = tile1 === TILE.POT ? c2 : c1;
      const otherR = tile1 === TILE.POT ? r1 : r2;
      const otherC = tile1 === TILE.POT ? c1 : c2;
  ...
  async processMatches(initialMatches) {
    ...
    while (matches.length > 0) {
      ...
      for (const m of matches) {
        ...
        const pts = m.cells.length * 10 * chainCount;
        this.score += pts;
        this.progress.score = this.score;
        this.globalStats.totalScore += pts;

        if (m.type === TILE.COFFEE) coffeeGroups++;
        if (m.type === TILE.PIE && m.size >= 5) {
          hasBigPie = true;
