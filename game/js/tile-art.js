...
export const CRYSTAL_PALETTES = {
  [TILE.COFFEE]: {
    base: '#3d2314', mid: '#8B5A2B', light: '#D4A574', rim: '#FFE4C4', glow: 'rgba(139,90,43,0.6)',
  },
  [TILE.PIE]: {
    base: '#B8860B', mid: '#FFD700', light: '#FFF8DC', rim: '#FFFACD', glow: 'rgba(255,215,0,0.7)',
  },
  [TILE.POT]: {
    base: '#1a1a1a', mid: '#3d3d3d', light: '#6a6a6a', rim: '#9a9a9a', glow: 'rgba(80,80,80,0.5)',
  },
  [TILE.PPT]: {
    base: '#6B3FA0', mid: '#B388FF', light: '#E1BEE7', rim: '#F3E5F5', glow: 'rgba(179,136,255,0.55)',
  },
  [TILE.READ]: {
    base: '#0d6b4f', mid: '#1ABC9C', light: '#7FEFD4', rim: '#A8FFEB', glow: 'rgba(46,204,113,0.55)',
  },
};
...
const ICON_DRAWERS = {
  [TILE.COFFEE]: drawCoffeeIcon,
  [TILE.PIE]: drawPieIcon,
  [TILE.POT]: drawPotIcon,
  [TILE.PPT]: drawPptIcon,
  [TILE.READ]: drawReadIcon,
};

/** 绘制完整水晶方块 */
export function drawCrystalTile(ctx, tile, x, y, size, stunned = false) {
  const palette = CRYSTAL_PALETTES[tile];
  const drawer = ICON_DRAWERS[tile];
  if (!palette || !drawer) return;

  const radius = size / 2;
  const extraGlow = tile === TILE.PIE;
...
/** 粒子用的小图标标签 */
export const TILE_PARTICLE_LABEL = {
  [TILE.COFFEE]: '☕',
  [TILE.PIE]: '✨',
  [TILE.POT]: '💥',
  [TILE.PPT]: '📉',
